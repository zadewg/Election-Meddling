...
def main(screenname, num):
	...
	data['SP'] = np.array([ analize_sentiment(data_preparation(tweet))[0] for tweet in data['Tweets'] ]) 
	data['SS'] = np.array([ analize_sentiment(data_preparation(tweet))[1] for tweet in data['Tweets'] ]) 

	display(data)

	pos_tweets = [ tweet for index, tweet in enumerate(data['Tweets']) if data['SP'][index] > 0]
	neu_tweets = [ tweet for index, tweet in enumerate(data['Tweets']) if data['SP'][index] == 0]
	neg_tweets = [ tweet for index, tweet in enumerate(data['Tweets']) if data['SP'][index] < 0]
	sbj_tweets = [ tweet for index, tweet in enumerate(data['Tweets']) if data['SS'][index] == 1]
	obj_tweets = [ tweet for index, tweet in enumerate(data['Tweets']) if data['SS'][index] == 0]

	print("\n\nPercentage of positive tweets:      {}%".format(len(pos_tweets)*100/len(data['Tweets'])))
	print("Percentage of neutral tweets:       {}%".format(len(neu_tweets)*100/len(data['Tweets'])))
	print("Percentage de negative tweets:      {}%".format(len(neg_tweets)*100/len(data['Tweets'])))
	print("Average subjectivity:               {}%\n\n\n".format(len(sbj_tweets)*100/len(data['Tweets'])))
...
#		print("\nSANITIZED SAMPLE: {}\n\n".format(data_preparation(tweet)))
	...
	C2OUT = (Ydrugs + Yabortion + Ylgbt + Yimmigration + Ywageup + Ytaxdown + Yenvironment + Ysocial + Ylaborunion + Yguns + Ytaxup + Ydeathpenalty + Ywar + Yhealthcare + Ywelfare + Ygovspend) / C2ACC
	#use mode then median instead


	OBJ = (len(obj_tweets)*10/len(data['Tweets']))
	...
	cax = divider.append_axes("right", size="5%", pad=0.2) #pad = separation, size = width
	plt.colorbar(scatters, cax=cax, ticks=[0, 1]).ax.set_yticklabels(['Subjective', 'Objective'])
...
if __name__ == "__main__":
	...
	for target in targets:
		...
		print("[*] Target: {} ({}) {} followers".format(target, location, num_followers))
